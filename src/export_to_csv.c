...
void export_to_csv(void) {
    ...
    fprintf(csv_file, "id,sensor_id,sensor_name,measured_at,pm25,pm10,co,no2,o3,so2\n");

    while (sqlite3_step(res) == SQLITE_ROW) {
        const char *sname = (const char *)sqlite3_column_text(res, 2);
        const char *mts = (const char *)sqlite3_column_text(res, 3);
        if (!sname)
            sname = "";
        if (!mts)
            mts = "";
        fprintf(csv_file, "%d,%d,\"%s\",%s,%.4f,%.4f,%.4f,%.6f,%.6f,%.6f\n", sqlite3_column_int(res, 0),
                sqlite3_column_int(res, 1), sname, mts,
                sqlite3_column_double(res, 4), sqlite3_column_double(res, 5), sqlite3_column_double(res, 6),
                sqlite3_column_double(res, 7), sqlite3_column_double(res, 8), sqlite3_column_double(res, 9));
    }
