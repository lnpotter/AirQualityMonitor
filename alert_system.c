...
void check_alerts(void) {
    ...
    const char *sql =
        "SELECT r.sensor_id, r.measured_at, r.pm25, r.pm10, r.co, r.no2, r.o3, r.so2 "
    ...
    d.pm25 = (float)sqlite3_column_double(st, 2);
    d.pm10 = (float)sqlite3_column_double(st, 3);
    d.co = (float)sqlite3_column_double(st, 4);
    d.no2 = (float)sqlite3_column_double(st, 5);
    d.o3 = (float)sqlite3_column_double(st, 6);
    d.so2 = (float)sqlite3_column_double(st, 7);

    sqlite3_finalize(st);
    aqm_db_close(db);

    if (exceeds_limits(&d)) {
        printf("ALERT: Latest sample exceeds configured limits (sensor %d, %s).\n", d.sensor_id, d.timestamp);
        ...
        printf("Latest reading is within configured limits (sensor %d, %s).\n", d.sensor_id, d.timestamp);
    }
}
